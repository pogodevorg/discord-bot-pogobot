    ...
    internal class GamblingModule : DiscordModule
    {
        ...
        public override void Install(ModuleManager manager)
        {
            manager.CreateCommands("", cgb =>
            {
                ...
                /*
                cgb.CreateCommand(Prefix + "report")
                    .Description($"Reports someone to a staff member. | `{Prefix}report @person reason`")
                    .Parameter("receiver", ParameterType.Unparsed)
                    .Parameter("reason", ParameterType.Required)
                    .Do(async e =>
                    {
                        var msgReason = e.GetArg("reason")?.Trim();
                        string msg = msgReason.ToString();
                        var mentionedUser = e.Message.MentionedUsers.FirstOrDefault(u =>
                                                            u.Id != NadekoBot.Client.CurrentUser.Id);
                        ...
                        if (mentionedUser == null)
                            return;
                    await e.Channel.GetUser(156487410115543040).SendMessage("[" + $"{e.User.Mention}" + "]: " + $"{mentionedUser.Mention}, {msg} " + "[" + format.ToString() + "]").ConfigureAwait(false);
                    });
                */
